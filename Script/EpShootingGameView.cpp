...
//	m_bmpBackground.LoadBitmap(IDB_BMP_BACKGROUND);
	...
	//Initialize 시나리오
	m_clSM.m_unit = &m_unit;
	m_clSM.m_backDC = &BackDC;
	m_clSM.m_enemyBullet = &m_enemyBullet;
...
void EpShootingGameView::OnTimer(UINT nIDEvent) 
{
	...
	for(i = 0; i < m_unit.GetSize(); i++)
	{
		short flg = m_unit[i]->CheckAndDraw(m_player.m_point);
		switch(flg)
		{
		case -1:
			//KillTimer(0);
	...
	if(m_RBtnDnFlg)
	{
		if(m_playerBullet.GetSize()<MAX_BULLETS)
		{
			...
			bullet->GiveAtt(pos, &BackDC, &m_unit);
